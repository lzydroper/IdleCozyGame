...
const SummonTab: React.FC<SummonTabProps> = ({ isOpen, onClose }) => {
  const { state, summonHero, summonTenHeroes } = useGame();
    ...
    <div className="absolute inset-0 z-50 bg-zinc-950 flex flex-col justify-between select-none overflow-hidden font-sans border-x border-zinc-900 shadow-2xl animate-in fade-in duration-200">
      ...
      <main className="relative flex-1 flex flex-col justify-between py-6 px-4 z-10">
        ...
        <div className="w-full flex items-center justify-between my-auto">
          ...
          <button
            onClick={() => setShowRulesModal(true)}
            className="w-13 h-13 rounded-2xl bg-gradient-to-b from-amber-400 to-amber-600 border-2 border-amber-200 flex flex-col items-center justify-center shadow-2xl shadow-amber-950/80 cursor-pointer hover:scale-105 active:scale-95 transition-all group"
            title="查看招募概率"
          >
            <HelpCircle className="w-7 h-7 text-amber-950 group-hover:scale-110 transition-transform" />
          </button>
        </div>
      ...
      <footer className="relative z-10 flex items-center justify-between p-3 bg-zinc-950/95 border-t border-amber-950/60 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Tab 2: 秘宝古卜 */}
          <button
            onClick={() => setInfoToastMessage('秘宝古卜功能即将开放！')}
      ...
      {/* 消息 Toast 提示 */}
      {infoToastMessage && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-[60] max-w-xs w-full px-4 py-2 rounded-xl bg-zinc-900/95 border border-amber-500/50 text-amber-300 text-xs font-bold shadow-2xl animate-in fade-in slide-in-from-top-2 duration-150 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Info className="w-4 h-4 text-amber-400 shrink-0" />
            <span>{infoToastMessage}</span>
          </div>
          <button
            onClick={() => setInfoToastMessage(null)}
            className="text-zinc-400 hover:text-zinc-200 shrink-0 ml-2"
          >
      ...
      {resultOutcomes && resultOutcomes.length > 0 && (
        <div className="absolute inset-0 z-[60] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-amber-500/50 rounded-3xl p-5 max-w-xs w-full shadow-2xl flex flex-col items-center gap-4 animate-in zoom-in-95 duration-200">
            <header className="text-center">
              <h3 className="text-base font-black text-amber-300 flex items-center justify-center gap-1.5">
                <Sparkles className="w-4.5 h-4.5 text-amber-400 animate-spin" />
                {resultOutcomes.length === 1 ? '招募获得' : '10 连招募获得'}
              </h3>
              ...
              {resultOutcomes.map((outcome, idx) => {
                ...
                if (outcome.arcaneOrbAwarded) {
                  return (
                    <div
                      key={idx}
                      className="flex flex-col items-center p-2.5 rounded-2xl bg-gradient-to-b from-amber-950/60 to-purple-950/60 border border-amber-400/80 shadow-lg text-center"
                    >
                      <Award className="w-8 h-8 text-amber-300 animate-bounce mb-1" />
                ...
                if (!config) {
                  return (
                    <div
                      key={idx}
                      className="flex flex-col items-center p-2.5 rounded-2xl bg-zinc-950/70 border border-zinc-800 text-center"
                    >
                      <Sparkles className="w-7 h-7 text-purple-400 mb-1" />
                ...
                return (
                  <div
                    key={idx}
                    className={`flex flex-col items-center p-2.5 rounded-2xl border relative overflow-hidden text-center shadow-lg transition-all ${
                      outcome.isNew
                        ? 'bg-gradient-to-b from-amber-950/70 to-zinc-950 border-amber-400'
                  ...
                  >
                    ...
                    <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-amber-500/40 overflow-hidden mb-1 flex items-center justify-center">
                      {config.avatar ? (
                        <img src={config.avatar} alt={config.name} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-lg font-black text-amber-300">{config.name[0]}</span>
                      )}
                    </div>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 font-black text-amber-950 text-xs shadow-xl active:scale-95 transition-all flex items-center justify-center gap-1 cursor-pointer hover:brightness-110"
            >
