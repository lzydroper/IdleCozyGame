...
export const HeroDetailModal: React.FC<HeroDetailModalProps> = ({
  ...
  const renderEquipSlot = (slot: EquipmentSlot, slotLabel: string, IconComponent: any) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-[8px] font-bold text-zinc-300 max-w-[48px] truncate text-center mt-0.5">
  ...
  const renderSkillSlot = (skillIndex: number) => {
    return (
      <div
        className="w-13 h-13 aspect-square rounded-xl border border-zinc-800 bg-zinc-950/60 flex flex-col items-center justify-center relative overflow-hidden"
        title={`技能 ${skillIndex}`}
      >
        <Flame className="w-4 h-4 text-purple-400/70" />
        <span className="text-[8px] font-bold text-zinc-400 mt-0.5">
          技能 {skillIndex}
        </span>
      </div>
    );
  ...
  const modalContent = (
    /* 对齐反馈 8: 叠加弹窗不再渲染暗色蒙版，使用 bg-transparent 防止黑度累积 */
    <div
      onClick={onClose}
    ...
    >
      <div
        ...
        className="bg-zinc-900 border border-zinc-750 rounded-2xl w-[94%] max-w-[380px] max-h-[85vh] p-3.5 flex flex-col gap-3 shadow-2xl overflow-y-auto"
      >
        {/* 顶部 Header: 左右切换按钮嵌入容器内部 (< 英雄详情 >) - 对齐反馈 5 */}
        ...
        <div className="grid grid-cols-3 gap-2 items-center bg-zinc-950/60 p-2.5 rounded-xl border border-zinc-800/80">
          ...
          <div className="flex flex-col items-center gap-1 text-center">
            ...
            {/* 动作控制 3 按钮对齐反馈 6:【一键装备/一键卸下】、【升级】、【升星/觉醒】 */}
            <div className="flex flex-col gap-1 w-full pt-1">
              {/* 按钮 1: 一键装备 / 一键卸下 */}
              <button
                onClick={handleToggleEquipAll}
                className="w-full py-1 rounded-lg text-[8px] font-black text-zinc-200 bg-zinc-800 hover:bg-zinc-750 border border-zinc-700 cursor-pointer active:scale-95 truncate"
              >
              ...
              <button
                ...
                className="w-full py-1 rounded-lg text-[8px] font-black text-amber-300 bg-amber-950/50 hover:bg-amber-900/60 border border-amber-500/40 cursor-pointer active:scale-95 truncate"
              >
                升级
              </button>
              ...
              {hero.star < STAR_MAX ? (
                <button
                  ...
                  className={`w-full py-1 rounded-lg text-[8px] font-black transition-all border cursor-pointer truncate disabled:cursor-not-allowed ${
                ...
                <button
                  disabled
                  className="w-full py-1 rounded-lg text-[8px] font-black bg-zinc-950 border border-zinc-800 text-zinc-500 cursor-not-allowed truncate"
                >
                  已觉醒
                </button>
              )}
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          {/* 右侧：基础属性显示 (对齐反馈 7: 严格使用 CONTEXT.md 6 项基础属性: 攻击、防御、生命、魔力、暴击、暴伤) */}
      ...
      {showTalentModal && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-zinc-900 border border-zinc-700 rounded-2xl w-[92%] max-w-[380px] max-h-[80vh] p-4 flex flex-col gap-3 shadow-2xl overflow-y-auto"
          >
