...
const WildernessTab: React.FC = () => {
  ...
  const handleStartExploration = (locationId: string | null) => {
    ...
    const foodCost = isRescue ? 15 : 10;
    const energyCost = isRescue ? 15 : 10;

    if (player.food < foodCost || player.energy < energyCost) {
      showToast(`生存指标过低（饱食度与魔能均需 >= ${foodCost}），请先补充！`, "error");
      return;
    }
  ...
  const handleMakeChoice = (choice: EventChoice) => {
    ...
    const nextHp = state.player.hp + (choice.results.stats?.hp || 0);
          ...
          <div className="flex flex-col gap-3">
            ...
            {rescueTargets.map(target => {
              const locationName = 
                ...
                target.realityLocationId === 'green_ruins' ? '古代温室废墟' : '高频信号塔';
