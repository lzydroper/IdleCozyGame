...
export function calculateEntityStats(params: CalculateStatsParams): CalculatedEntityStats {
  ...
  const extraAttack = primary.strength * PRIMARY_STAT_SCALING_CONFIG.STRENGTH_TO_ATTACK;
  const extraCritDmg = primary.strength * PRIMARY_STAT_SCALING_CONFIG.STRENGTH_TO_CRIT_DMG;

  const extraMaxHp = primary.constitution * PRIMARY_STAT_SCALING_CONFIG.CONSTITUTION_TO_MAX_HP;
  const extraDefense = primary.constitution * PRIMARY_STAT_SCALING_CONFIG.CONSTITUTION_TO_DEFENSE;

  const extraCritRate = primary.agility * PRIMARY_STAT_SCALING_CONFIG.AGILITY_TO_CRIT_RATE;
  const critResist = primary.agility * PRIMARY_STAT_SCALING_CONFIG.AGILITY_TO_CRIT_RESIST;

  const extraMaxMp = primary.intelligence * PRIMARY_STAT_SCALING_CONFIG.INTELLIGENCE_TO_MAX_MP;
  const extraArcaneBoost = primary.intelligence * PRIMARY_STAT_SCALING_CONFIG.INTELLIGENCE_TO_ARCANE_BOOST;

  const durationReduction = primary.willpower * PRIMARY_STAT_SCALING_CONFIG.WILLPOWER_TO_DURATION_REDUCE;
  const effectReduction = primary.willpower * PRIMARY_STAT_SCALING_CONFIG.WILLPOWER_TO_EFFECT_REDUCE;

  const cooldownReduction = primary.transcendence * PRIMARY_STAT_SCALING_CONFIG.TRANSCENDENCE_TO_COOLDOWN_REDUCE;
  ...
  const finalCritRate = Math.min(
    ...
    Math.max(BUFF_LIMIT_CONFIG.MIN_CRIT_RATE, base.critRate + extraCritRate)
  );
  const finalCritDmg = Math.max(BUFF_LIMIT_CONFIG.MIN_CRIT_DMG, base.critDmg + extraCritDmg);
