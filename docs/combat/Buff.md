...
buff拥有触发时机，这是一个数组，可以指定某些单位的某些时机，并注册相关所有效果到对应时机中，效果拥有是否可触发接口，时机传入当前回合归属来得到结果；也可能不是轮次时机而是某个技能发动时
...
- 眩晕
	...
	- Description: 令目标眩晕{1}回合
					- 眩晕：跳过当前回合
    - 备注：这里的1的效果可能在目标回合开始前结算时，因为目标意志属性过高变成0，导致眩晕buff提前结束，目标回合不跳过
- 折焰
	- Duration: forever
	- Trigger: 目标攻击后
	- Renew: false
...
- 春和
	...
	- 备注：该buff的生效需要效果有被识别字段、Modifier
```
